...
const byte FINGER  =4; //move fingers
...
void setup() {
  ...
  pinMode(servoport4, OUTPUT);
  pinMode(servoport5, OUTPUT);
...
void handleSerialData(char inData[], byte index) {
  ...
  else if (strcmp(words[0], "demo") == 0) { // Send demo pattern
          struct payload myPayload = {DEMO, '\0', {'\0'}};
          size_t len = sizeof(DEMO) + sizeof('\0') + sizeof('\0');

          broadcast(myPayload, len);
        }
   
   else if (strcmp(words[0], "ring") == 0) { // Send ring pattern
       byte pattern = (byte) 3;
          struct payload myPayload = {LED, pattern, {'\0'}};
          size_t len = sizeof(DEMO) + sizeof('\0') + sizeof('\0');
...
void handlePayload(struct payload * myPayload) {
  switch(myPayload->command) {
    ...
    case FINGER:
        Serial.println("moving a finger");
        fingerMove(myPayload->led_pattern);
        break;
...
void fingerMove(byte pattern){
  switch(pattern){
    
    case 0:
      Serial.println("close all fingers");
      closeAll();
      break;
      
    case 1:
      Serial.println("open pinky");
      closeAll();
    ...
    case 3:
      Serial.println("open middle");
      closeAll();
      for (pos = middleMin; pos < middleMax; pos +=1){
        middle.write(pos);
        delay(3);
      }
      break;
      ...
      case 4:
        Serial.println("open pointer");
        closeAll();
        for (pos = pointerMin; pos < pointerMax; pos +=1){
          pointer.write(pos);
          delay(3);
        }
        break;
      ...
      case 5:
        ...
        closeAll();
        for (pos = thumbMin; pos < thumbMax; pos +=1){
          thumb.write(pos);
          delay(3);
        }
        break;
...
void broadcast(struct payload myPayload, size_t len){
  uint16_t completion = 0;
  Serial.print("Broadcasting... this will take a bit\n");
  radio.stopListening();
    digitalWrite(SROEPin, LOW);
  for(int i=0; i < 600; i++){
    setValue(i);
    ...
    digitalWrite(SROEPin, HIGH);
  radio.startListening();
  Serial.print("Finished broadcasting\n");
}
